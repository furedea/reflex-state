...
ReflexState v0.1 は，Pi のコーディングセッションの実行状態を型付きデータとして管理します．
ファイルの変更，検証結果，作業フェーズはコードで抽出します．任意で有効にできる
TypeSafe Jev は，ツールエラーの分類・解消やタスクの完了を判定します．
状態の変更は reducer が担当します．根拠はサイズ制限付きの生イベントに結び付け，
自然言語の要約は生成しません．
